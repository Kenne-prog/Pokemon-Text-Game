...
public class Trainer extends Entity{
  ...
  public void usePotion(int pokeIndex){
    ...
    if (potions > 0){
      if (p.getHp() < p.getMaxHp()){
        p.heal(); //heal pokemon
        pokemon.set(pokeIndex,pg.addRandomBuff(getPokemon(pokeIndex)));
  ...
  */
  ...
  /**
  * Buffs all the pokemon
  */
  public void buffAllPokemon(){
    PokemonGenerator pg = PokemonGenerator.getInstance();
    for (int i = 0; i < pokemon.size(); i++){//iterates through pokemon by index
      Pokemon p = pokemon.get(i);
      p = pg.addRandomBuff(p);
      pokemon.set(i, p);
    }
  }
  /**
  * Debuffs the current pokemon with the current index.
  * @param int index is the index of the pokemon on the 
  * ArrayList
  */
  public void debuffPokemon(int index){
    PokemonGenerator pg = PokemonGenerator.getInstance();
    Pokemon p2 = getPokemon(index);
    pokemon.set(index, pg.addRandomDebuff(p2));
  }
  /**
  ...
  */
  ...
  /**
  * removePokemon - removes a pokemon that is at a certain index.
  * @param - the index of the pokemon to remove.
  * @return - the pokemon to remove.  
  */
